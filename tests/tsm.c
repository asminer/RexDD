...
#define BUF_SIZE 2048   // minterms buffer size
...
/*-----------------------------------------------Restriction--------------------------------------------*/
bool has_common_tsm(rexdd_forest_t* F, rexdd_edge_t* e1, rexdd_edge_t* e2)
{
    //
    if (rexdd_edges_are_equal(e1, e2)) return true;
    if (rexdd_is_terminal(e1->target) && rexdd_terminal_value(e1->target) == DC_VAL) return true;
    if (rexdd_is_terminal(e2->target) && rexdd_terminal_value(e2->target) == DC_VAL) return true;
    if (rexdd_is_terminal(e1->target) && rexdd_is_terminal(e2->target)) return false;
    ...
    if (lvl1 < lvl2) {
        ...
        cmp0 = has_common_tsm(F, e1, (e2->label.swapped)? &he2 : &le2);
        cmp1 = has_common_tsm(F, e1, (e2->label.swapped)? &le2 : &he2);
    } else if (lvl1 > lvl2) {
        ...
        cmp0 = has_common_tsm(F, (e1->label.swapped)? &he1 : &le1, e2);
        cmp1 = has_common_tsm(F, (e1->label.swapped)? &le1 : &he1, e2);
    } else {
        rexdd_unpack_low_edge(rexdd_get_packed_for_handle(F->M, e2->target), &le2.label);
        le2.target = rexdd_unpack_low_child(rexdd_get_packed_for_handle(F->M, e2->target));
        rexdd_unpack_high_edge(rexdd_get_packed_for_handle(F->M, e2->target), &he2.label);
        ...
        cmp0 = has_common_tsm(F, (e1->label.swapped)? &he1 : &le1, (e2->label.swapped)? &he2 : &le2);
        cmp1 = has_common_tsm(F, (e1->label.swapped)? &le1 : &he1, (e2->label.swapped)? &le2 : &he2);
    }
...
rexdd_edge_t common_tsm(rexdd_forest_t* F, rexdd_edge_t* e1, rexdd_edge_t* e2, uint_fast32_t n)
{
    rexdd_edge_t ans;
    // Base cases:
    if (rexdd_edges_are_equal(e1, e2)) {
        rexdd_set_edge(&ans,
                        e1->label.rule,
                        e1->label.complemented,
                        e1->label.swapped,
                        e1->target);
        return ans;
    }
    if (rexdd_is_terminal(e1->target) && rexdd_terminal_value(e1->target) == DC_VAL) {
        rexdd_set_edge(&ans,
                        e2->label.rule,
                        e2->label.complemented,
                        e2->label.swapped,
                        e2->target);
        return ans;
    }
    if (rexdd_is_terminal(e2->target) && rexdd_terminal_value(e2->target) == DC_VAL) {
        rexdd_set_edge(&ans,
                        e1->label.rule,
                        e1->label.complemented,
                        e1->label.swapped,
    ...
    if (lvl1 < lvl2) {
        ...
        tmp.edge[0] = common_tsm(F, e1, (e2->label.swapped)? &he2 : &le2, lvl2-1);
        tmp.edge[1] = common_tsm(F, e1, (e2->label.swapped)? &le2 : &he2, lvl2-1);
    } else if (lvl1 > lvl2) {
        tmp.level = lvl1;
        rexdd_unpack_low_edge(rexdd_get_packed_for_handle(F->M, e1->target), &le1.label);
        le1.target = rexdd_unpack_low_child(rexdd_get_packed_for_handle(F->M, e1->target));
        ...
        tmp.edge[0] = common_tsm(F, (e1->label.swapped)? &he1 : &le1, e2, lvl1-1);
        tmp.edge[1] = common_tsm(F, (e1->label.swapped)? &le1 : &he1, e2, lvl1-1);
    } else {
        tmp.level = lvl1;
        rexdd_unpack_low_edge(rexdd_get_packed_for_handle(F->M, e2->target), &le2.label);
        le2.target = rexdd_unpack_low_child(rexdd_get_packed_for_handle(F->M, e2->target));
        ...
        tmp.edge[0] = common_tsm(F, (e1->label.swapped)? &he1 : &le1, (e2->label.swapped)? &he2 : &le2, lvl1-1);
        tmp.edge[1] = common_tsm(F, (e1->label.swapped)? &le1 : &he1, (e2->label.swapped)? &le2 : &he2, lvl1-1);
    }

    rexdd_edge_label_t l;
    l.rule = e1->label.rule;
    l.complemented = 0;
    l.swapped = 0;
...
rexdd_edge_t tsm(rexdd_forest_t* F, rexdd_edge_t* root, uint_fast32_t n)
{
    rexdd_edge_t ans;
    // terminal case
    if (rexdd_is_terminal(root->target)) {
    ...
    if (has_common_tsm(F, &lc, &hc)) {
        //
        rexdd_edge_t child_common = common_tsm(F, &lc, &hc, levelc-1);
        // since root is X and pushing complement bit down
        return tsm(F, &child_common, n);
    }

    // new node
    rexdd_unpacked_node_t tmp;
    tmp.level = levelc;
    tmp.edge[0] = tsm(F, &lc, tmp.level-1);
    tmp.edge[1] = tsm(F, &hc, tmp.level-1);
...
/*----------------------------------------Mark and count ALL root---------------------------------------*/
    ...
    /*  clear unmarked nodes by setting to 0*/
    for (q=0; q<F.M->pages_size; q++) {
        const rexdd_nodepage_t *page = F.M->pages+q;
        for (n=0; n<page->first_unalloc; n++) {
            if (!rexdd_is_packed_marked(page->chunk+n)) {
                (page->chunk+n)->first64 = 0;
                (page->chunk+n)->second64 = 0;
                (page->chunk+n)->third32 = 0;
                (page->chunk+n)->fourth32 = 0;
            }
        } // for n
    } // for p
...
/*--------------------------------------------Concretizing----------------------------------------------*/
    ...
    for (int i=0; i<5*(argc-1); i++) {
        ...
        if (root_flag[i]==1) root_edge[i] = tsm(&F, &root_edge[i], F.S.num_levels);
        fp = fopen("tsm_process.txt", "w+");
    ...
    for (int i=1; i<argc; i++) {
        ...
        printf("number of nodes (%s) in forest %d after concretizing (tsm) is %llu\n", TYPE, i, num_nodes);
    }
    if (argc>2) {
        unmark_forest(&F);
        ...
        printf("Total number of nodes (%s) in forest after concretizing (tsm) is %llu\n", TYPE, num_nodes);
    }
